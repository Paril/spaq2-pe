...
edict_t *medic_FindDeadMonster(edict_t *self)
{
    ...
    while ((ent = findradius(ent, self->s.origin, 1024)) != NULL) {
        ...
        if (ent->nextthink)
            continue;
        if (!visible(self, ent, MASK_OPAQUE))
            continue;
        if (!best) {
            best = ent;
            continue;
...
void medic_fire_blaster(edict_t *self)
{
    ...
    monster_fire_blaster(self, start, NULL, 2, 1000, MZ2_MEDIC_BLASTER_1, 0);
}
...
void medic_cable_attack(edict_t *self)
{
    ...
    tr = gi.trace(start, NULL, NULL, self->enemy->s.origin, self, MASK_SHOT | CONTENTS_DEADMONSTER);
